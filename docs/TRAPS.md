...
- **A value beside a wrapping title floats against the wrap, not the name.** `DateRow` centered its countdown and "63 days" sat level with nothing; `DenseRow` still does it, and on Money a three line block puts the amount beside the second line. Top align where the block can be taller than the value. **`DenseRow` is not changed**, because it is the most used row in the app and centering is right for the ordinary one line case. #376.
