...
- **Check unlocked first**: `adb shell dumpsys window | grep isKeyguardShowing`. A secure keyguard fails every instrumented class with `IllegalStateException: No compose hierarchies found in the app`, which blames your test. No way past it from here. #316. **It also locks part way through a 15 minute run**, and then it is 3 failures in 725 rather than all of them, in unrelated classes, which reads as a real regression and is not: 2026-08-15, `CaptureTest`, `CaptureProjectTest`, `AppointmentsMonthTest`, all three green in isolation and the whole suite green on a re-run. **Same message, same cause, and the small number is what makes it convincing.** **Another app stealing the foreground does the same thing without the keyguard**: 2026-08-16, a different app on the device came to the front twice mid-suite, 15 failures across nine unrelated classes, XML truncated at 711 of 728, every one green in isolation. Check `dumpsys window mCurrentFocus` when the keyguard says no.
- **A seed that fails leaves an empty notebook and every screenshot after it is a lie.** 2026-08-16: `tools/seed.sh` tapped the restore confirm button at a position below the fold, which does nothing and reports nothing, so three screens were assessed while showing their empty states before anybody noticed the data was gone. **The script now scrolls, and it already says which happened**: it prints "Restored." or "The restore did not finish", and that line is the thing to read before trusting any capture. The restore screen grows whenever its copy grows, so this will recur; the tell is a filled button sliced by the bottom edge of the viewport. The app was fine both times.
...
- **A value beside a wrapping title floats against the wrap, not the name.** `DateRow` centered its countdown and "63 days" sat level with nothing; `DenseRow` still does it, and on Money a three line block puts the amount beside the second line. Top align where the block can be taller than the value. **`DenseRow` takes `trailingAtTop` rather than changing its default**, because it is the most used row in the app and centering is right for the ordinary one line case. A caller whose title can wrap asks for it and nothing else moves. #376.
...
- **Any label that went through `Bidi.isolate` carries `\u2068` and `\u2069`.** Chip names, card tabs, row titles. `onNodeWithText("Person 1")` matches nothing and the failure says "assertExists failed", which reads as the screen not drawing it. Match `Bidi.isolate(name)` or strip the marks. **This trap was written down and then walked into an hour later**, so match on the isolated form by habit rather than by remembering.
- **A spoken label built from a card's tab carries isolate characters.** `Bidi.join` wraps every part in `\u2068` and `\u2069`, so "Remove Next up" never matches the description, which is really "Remove ⁨Next up⁩". Assert on a test tag, or strip the marks the way `TodayFieldScreenTest.spoken` does. The failure reads "is not displayed" rather than "not found", which sends you looking at the layout.
...
- **And the same thing the other way round: a field with a writer and no reader.** `MANIFEST.json` has always carried `readable.pages`, the archive writer has always filled it in, and the manifest parser takes `locale` out of that object and leaves the count behind, so `Manifest.readablePages` came back `0` for every archive this app has ever made. Nothing noticed for the same reason nothing notices the other direction: **the only thing that finds it is code that finally reads the value.** #412's export readback was its first caller, and the symptom was the export refusing a perfectly good archive on the signed release build. **Grep both sides of a field, not only both sides of a foreign key.**
- **A check that reads back through a write grant is not a check you can fail somebody on.** The first version of the export readback reopened the SAF destination through the URI it had just written to, and treated any failure as a failed export. Telling somebody their export failed while a valid archive sits on their disk is worse than the defect the check exists to fix, because the file it disparages is their only way back. Validate where the bytes are known to be readable, and check the destination for length.
- **On the phone, bisect. Do not reason.** Two rebuilds were spent guessing why the export failed on the release build. Making the new check non-fatal, watching it say Saved, and reading the logged exception took one rebuild and named the cause exactly. The release build strips almost nothing useful from `Log.w`.
