...
fun NotebookShell(
    ...
    /**
     * Everything on screen right now, in one object rather than 214
     * remembered locals. `ShellState` says why; `DECISIONS.md` B6 says what it
     * cost to find out. `with(ui)` below is what keeps every reference to
     * these reading exactly as it did.
     */
    val ui = remember { ShellState() }
     ...
     */
    // **The state itself rather than only the delegate**, for these two, because
    // an extracted overlay group needs the same one the shell is holding. A
    // second `rememberSaveable` inside the group would be a second answer to
    // "is setup open", and the two would disagree the first time either changed.
    val captureDraftState = rememberSaveable(stateSaver = CaptureFormState.Saver) {
        mutableStateOf(CaptureFormState())
    }
    var captureDraft by captureDraftState
    val setupOpenState = rememberSaveable { mutableStateOf(false) }
    ...
    with(ui) {
        ...
        BackHandler(enabled = restoreOpen) { restoreOpen = false }
        ...
        Surface(
            ...
            if (exportOpen) {
                ...
                // Read when the screen opens rather than held for the life of
                // the shell, so a copy saved on another screen is reflected.
                LaunchedEffect(exportOpen) {
                    lastExportAt = repository.settingTimestamp(Repository.KEY_LAST_EXPORT)
                }
            }
            ...
            if (exportTarget != null) {
                LaunchedEffect(exportTarget) {
                        ...
                        runCatching {
                            ...
                            try {
                                ...
                                context.contentResolver.openOutputStream(exportTarget)?.use { out ->
                                    ...
                                    // **Pushed to the disk rather than left in a
                                    // buffer.** #412: no sync() existed anywhere
                                    // in main, so "Saved" could mean "handed to
                                    // the operating system and not yet written".
                                    out.flush()
                                ...
                                try {
                                    ...
                                    // which is to say the whole record in the
                    ...
                    if (outcome.isSuccess) {
                        // **Recorded only once the readback has passed.** #413
                        // and #412: the date means an archive that opens, not
                        // an attempt that was made.
                        repository.putSettingTimestamp(
                            Repository.KEY_LAST_EXPORT, System.currentTimeMillis(),
                        )
                        lastExportAt = repository.settingTimestamp(Repository.KEY_LAST_EXPORT)
                    }
                    exportState =
            ...
            // **The whole restore, in one composable that owns its own state.**
            // #343 and B6: the file, the passphrase, the two guards and the choice
            // between replacing and merging are used by nothing else here, and the
            // unrecoverable screen needs the same flow somewhere the shell cannot
            // reach. Six state variables and about a hundred and twenty lines left
            // this composable with it.
            if (restoreOpen) {
                RestoreFlow(
                    onBack = { restoreOpen = false },
                    onApplied = {
                        // Everything on screen came from the database that was
                        // just replaced, so it is all reread rather than left
                        // showing the notebook that no longer exists.
                        revision += 1
                        recountConflicts = true
                    },
                )
            }

            // A merge may have decided something, and the door to reading what it
            // decided only exists when there is something behind it. Counted at the
            // one moment the number can change rather than polled.
            if (recountConflicts) {
                LaunchedEffect(revision) {
                    unseenConflicts = Repository.open(context).unseenConflicts()
                    recountConflicts = false
                }
            }
                ...
                LaunchedEffect(medicationDraft) {
                    ...
                    if (correcting != null && medicationDraft.name.isNotBlank()) {
                        repository.updateMedication(
                            ...
                            frequencyText = medicationDraft.frequency,
