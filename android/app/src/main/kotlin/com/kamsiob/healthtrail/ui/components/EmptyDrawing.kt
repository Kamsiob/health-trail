 ...
 *
 * **The mark alone, since 2026-08-16.** It carried a "ground" under it, the
 * trail map drawn small, and the owner saw "weird little squiggly lines...
 * drawings that you just made up." He was right in the way that matters: a
 * metaphor that needs explaining is clutter. The mark is the section's own
 * icon from the table of contents, scaled up rather than redrawn. **Nothing here was
...
fun EmptyDrawing(
    ...
    // **A place outside the twelve used to get the ground alone; with the
    // ground gone it gets nothing, and honestly nothing.** Reserving the box
    // with no drawing in it would be rule 11's blank area in miniature on
    // every screen that passes null.
    ...
    Box(
        ...
        // **The ground is gone, on the owner's order, 2026-08-16.** It was
        // the trail map drawn small under the mark, per 5.17, and what he saw
        // was "weird little squiggly lines... some kind of drawings that you
        // just made up." A metaphor that has to be explained is not a
        // metaphor, it is clutter, and the person it read as clutter to is
        // the one the app is for. The section's own icon stays, because it
        // teaches the mark the person navigates by; the landscape under it
        // taught nothing.
        if (mark != null) {
            Canvas(
