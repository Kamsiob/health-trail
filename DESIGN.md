...
| Correcting an incident | 09 One entry | The grid draws Edit in an incident's header and draws nothing behind it, because until 2026-08-12 nothing could correct one: the only `UPDATE incident` set `resolved_at`. **Built 2026-08-12**, #358. **It only ever corrects and never creates**, which is why it is not an add screen with an `existing` parameter like the others: an incident is born in the capture form with its first entry beside it in one transaction, and a second way to make one would be a second answer to a question that form already asks. **The words, the date and the note, and nothing else**: what happened next is entries, and they are corrected where entries are corrected. **The date keeps whatever precision was given** and uses the same picker and the same "not sure" as every other date, per rule 17. Review at #359 |
| Starting a care thread | 23 Track something new | The grid draws no way to start a thread, because until 2026-08-12 there was none: a situation template was the only thing that had ever created one. **Built 2026-08-12**, #349, decided in D145. **One question and nothing else**, per law 3, so it is the naming face of screen 23 without the catalog above it: there is no catalog of threads to pick from and inventing one would be the app deciding what kinds of care there are. **A name is the only required field**, rule 13, and the start date is today rather than asked, because a thread somebody starts today started today. **Reached from the foot of the list rather than the header**, where naming your own measure already sits, and **from the empty state's own action**, because a notebook with no situation applied has no threads at all and that is exactly where somebody meets this. Review at #350 |
...
| One tracked thing, its own screen | **24 Tracking one thing** | Drawn, so this is what the drawing does not carry: how it is reached and what happens to the rest of the page. **Built 2026-08-18**, #398. Every measure is one tap from Progress and has its own screen; before this only whichever measure happened to be the hero had a chart, and tapping another promoted it to hero rather than opening it. **Progress leads with the one written in most recently and lists the rest as rows**, the same grammar D205 gave the projects list. **The flat "every reading" list is gone**, because it interleaved every measure's readings into one stream. `docs/TRACKED-THINGS.md` is the brief and its section 7 is the system; **three of its five shapes need schema the contract does not have and are on #403**. Review at #404 |
...
| Memos, and one memo | **Undrawn entirely** | The grid draws no notes section, because there was none until the owner asked for one on 2026-08-18. **Built 2026-08-18**, #397, D207. **A memo is an entry**, `kind = 'note'`, so it is already on the trail, in search, in the archive and in the merge; the screens are a lens on it. **The memos page is a grid or a list**, the person's choice, remembered per section, and it drops to one column above font scale 1.5 because two columns of large type is two gutters. **The memo screen's three marks are a toolbar bound to the field**, and **it is the one screen in the app with no microphone**, at the owner's word. **`MemosAbout` is rule 18's other half on six kinds of thing.** Review at #406 |
| Deleted Items | **Undrawn entirely** | Nothing drew a bin, because rule 3 made one possible without anybody noticing: deletion is always a tombstone, so the rows were already kept and nothing could read them. **Built 2026-08-18**, #405, **renamed and given a permanent delete by the owner 2026-08-27**, #465. **Restore clears one column** and the thing returns to its own place on the timeline, because the row kept its own dates. **Delete permanently is the one write in the app that is not a tombstone**: the row, everything that only existed because of it, its change log entries and its attachment bytes, behind a confirmation that says it cannot be undone. `contract/DATA-CONTRACT.md` 3 is the rule. The claim that used to sit here, that there is no delete forever because the archive is the record, cited D24, which is about the Keystore and says nothing of the kind. **The door is always on More**, unlike the conflict door, because its value is knowing it exists before you need it. Review at #406 |
| The projects list, reimagined | **Projects grid, screen 02** | Drawn, so this is what the drawing does not carry: a lead. **Rebuilt 2026-08-18**, #399, D205. The grid draws peers with state and the build made them five identical cards, so nothing led and the eye landed nowhere. **One project leads**, whichever live one has the nearest date somebody else set, drawn as a tonal block with the road full width and its labels on, which is the one place on this screen the road can be read; with no dated project it is the one started most recently and leads with its standing. **Everything else is a dense row**, per rule 22, with the road's information as words. **The uppercase status tag above every name is gone**, which is where GOV.UK's two years of research on that exact pattern came out. Review at #401 |
| A project's file, as four tiles | **Projects grid, screen 05** | Drawn, so this is the part the drawing does not carry: how the other screens are reached. **Rebuilt 2026-08-18**, #399, D206. Five dense rows, one of which was settings, plus two loose tonal pills at the foot of the scroll, is what the owner called menus and sub menus. **Four tiles**, rule 22's component for a fixed set of destinations, each with its own hue from `hueFor`, its name and what it counts. **Setup, the name and the removal moved to a Material overflow in the top bar**, which is three things off the page and one door fewer. **The answer and the date became one block**, because they were two tonal cards a gap apart with the second drawn louder, so the screen had two possible leads. Review at #401 |
...
| Saving a document | 05 Capture that forgives | The grid draws the capture form staged and draws no add form at all, and this one predates law 3 being enforced. **Rebuilt 2026-08-12**, #361, decided in D147. **Three questions**: the paper, what it is and when it is from, then where the original went, with the folder and the note behind the same "Add more" the capture form's third stage uses. **The first question is an empty sheet at the size of the screen**, per rule 22, and the sheet is the control: a title, a button and two thirds of nothing was what it was before, which rule 11 rules out. **The sheet is squarer than real paper** because at A4's ratio the size limit fell below the fold. **Correcting shows the whole record at once and opens the disclosure**, D147. Review at #362 |
| Adding somebody to the care team | 05 Capture that forgives | Undrawn, and law 3 applies. **Rebuilt 2026-08-12**, #361, **and corrected 2026-08-13**, #376. **The name, the role and the number lead, in that order, in one group**; where they work and anything to remember are behind "Add more", and the role chips sit under the group rather than inside it. The problem this entry described was that the role and up to six chips sat *between* the name and the number, so the number was the field furthest down; putting the three together and the chips below solves that without hiding the role, which an earlier build did and had reverted for its own reason. **Three records disagreed about this and all three are now the same.** **No stages**: three fields is not a conversation and `StageDots` is for something walked in order. Review at #363 |
| Adding a medication | 05 Capture that forgives | Undrawn, and it was four multi line fields down one scroll. **Rebuilt 2026-08-12**, #361. **The name, the dose and the emergency card question lead**; what it is for and any other note are behind "Add more". **The card question stays out of the disclosure on purpose**: the moment somebody writes a medication down is the moment they know whether it matters in an emergency, and a question folded away is a question nobody answers. Review at #364 |
| Adding an appointment | 05 Capture that forgives | Undrawn. **Rebuilt 2026-08-12**, #361. **What it is and roughly when lead**; where it is and the note are behind "Add more", because the address usually arrives on a letter days after the call that made the appointment. Review at #365 |
| Adding a bill | 05 Capture that forgives | Undrawn. **Rebuilt 2026-08-12**, #361. **What it is, how much, and where it stands** are the bill and all three stay; the open note box is behind "Add more". Review at #366 |
...
**And the label now answers to what is in the field**, decided 2026-08-12 under rule 10 rather than escalated: **"Skip this" while the stage is empty and "Next" once it has something.** The rule that made it "Skip this" is intact, since nothing in capture is required and the control says so on the stage where nothing has been typed, and **the oddness is gone**: "Skip this" was describing something the person was plainly not doing the moment they typed a line. `capture.next` had been sitting in all four catalogs unused, which is what suggested it had been considered once.

**Screen 03, the notebook's fold, which opens itself where there is room.** The grid draws four sections leading and the rest behind one fold, and that is right on a short screen. **On a tall one it left the page stopping before the middle**, with eight rows folded away and half the screen empty, which is the app being tidy at the person's expense. The fold now starts open when the screen is taller than the closed notebook and starts closed when it is not, and **it is still a fold**: somebody who closes it keeps it closed, and the order of the sections never changes either way. Rule 23, and 2026-08-12, on the owner's observation that the app was not using the whole screen. **Asked of the configuration rather than measured in layout**, because a `BoxWithConstraints` answers during measurement and killed a journey test with "performMeasureAndLayout called during measure layout".

**Screen 17, what they take, and where the count says it comes from.** The grid writes "3 · from Medications" in the fold's count. **In this app the count is a pill: mono, tabular, and fully rounded**, because a count is data, and a phrase in it wraps to three lines and is clipped by the pill's own shape at font scale 2.0. Seen on the phone on 2026-08-12 rather than reasoned about, first as a label crushed to a three character column and then, after giving both sides weight, as a count clipped by its own ellipse. **The count is the number and the source is said inside the fold when it opens**, which is where somebody who wants to correct a medication is actually looking. #354.
