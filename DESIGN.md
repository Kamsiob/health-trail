...
| What you took out | **Undrawn entirely** | Nothing drew a bin, because rule 3 made one possible without anybody noticing: deletion is always a tombstone, so the rows were already kept and nothing could read them. **Built 2026-08-18**, #405. **Restore clears one column** and the thing returns to its own place on the timeline, because the row kept its own dates. **There is no delete forever**, deliberately: the archive is the record, D24. **The door is always on More**, unlike the conflict door, because a bin's value is knowing it exists before you need it. Review at #406 |
