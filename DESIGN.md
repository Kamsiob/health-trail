...
**Every touchable node says what it is, and the build checks it.** `ScreenReaderTest` walks a screen's semantics tree, in every window including a sheet's own, and fails on any node carrying a click that has neither text nor a content description. **It walks 54 of the 75 screens, not all of them**, and the number is here rather than a claim of completeness because a document that overstates a check is worse than a missing check: the next person reads the claim and stops looking. **#342** is the gap and the twenty-one it does not reach.
