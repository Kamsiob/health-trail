...
- **One month, gathered, and every line opens where it came from.** Tap a month heading on the trail and that month arrives composed: what somebody marked as worth remembering, where they were, the appointments, what went wrong and what was answered, the paperwork, and everything written down. Nothing in it is summarized or inferred. Every row is the row that already exists elsewhere in the notebook, gathered by when it happened, and a month can be sent to somebody who is not there.
- **The path so far.** The milestones somebody decided were worth marking, oldest first on a gold trail, each naming the place it happened. The app never marks one and never works one out from a count or a measurement.
...
- **The papers of a project, split by what they sent and what you sent**, which is the one distinction that matters in any process. An empty place says it is waiting, never that it is missing, and nothing counts the empty ones.
- **Each project keeps its own trail**, read forwards: what was said, where the road turned, and the dates it is running against, on one line, with the quiet stretches drawn and dated. Months where nothing happened are stated as fact, never as something you let slip.
