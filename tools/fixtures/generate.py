...
class Generator:
    ...
    def milestones(self, db, subject_id, chapters=()):
        """The moments somebody decided were worth marking.

        **Each one names the chapter it fell in**, #237. `milestone.chapter_id`
        is in the schema and the arc reads it, and no fixture ever wrote one, so
        a built link had never been seen on a screen: every milestone in every
        walk was a moment with no place.
        """
        for _ in range(self.scaled(FULL["milestones"])):
            ...
            if chapters:
                # The chapter the day falls in, by position rather than by
                # date arithmetic: the chapters are generated in order and
                # divide the record evenly, which is what puts a milestone in
                # the place it happened rather than in a random one.
                span = max(1, self.days // max(1, len(chapters)))
                values["chapter_id"] = chapters[min(day // span, len(chapters) - 1)]
            self.row(db, "milestone", values, day=day)
    ...
    def documents(self, db, subject_id, chapters, incidents=()):
        ...
        """
        # The entries that belong to something raised, in a fixed order so one
        # seed keeps producing one notebook.
        raised = [
            row[0]
            for row in db.execute(
                "SELECT id FROM entry WHERE incident_id IS NOT NULL ORDER BY id",
            ).fetchall()
        ]
        ...
        for index in range(wanted):
            ...
            document_id = self.row(
                ...
                "document",
                {
                    ...
                    # **Every third paper came out of something that was
                    # raised.** A document reaches an incident through the
                    # entry it was written against, which is how the incident
                    # screen finds its paperwork, and no seed had ever produced
                    # one: the link was built at both ends and visible at
                    # neither.
                    "entry_id": raised[index % len(raised)] if raised and index % 3 == 0 else None,
