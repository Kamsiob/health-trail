...
**Last rewritten:** 2026-08-11, after a long night: #218, #135, #339, #340, and the audits on #204, #205 and #46.
...
- **598 instrumented tests pass**, full run 2026-08-11 at 02:03 on the unlocked phone. **Six classes are new or much bigger**: `RemovalIsVisibleTest` seventeen, `FirstDaysTest` seven, `DocumentDateTest` four, `IncidentScreenTest` two, `OwnMeasureTest` three, and **`ScreenReaderTest` from 61 cases to 92**, covering every screen.
...
- **Milestone 4, the rest of the v4 conversion: ten issues left.** #324, #231, #132 and #221 closed on 2026-08-10, and **#218, #135, #339 and #340 on 2026-08-11**. **Three new ones came out of the night's work and each says exactly what it needs**: #341 is three screens printing their own sentence twice and needs copy in four languages, #342 is `ScreenReaderTest` walking 44 of 75 screens while `DESIGN.md` 12 claims every one, and #339 is closed but #57 still needs an owner decision about the camera.
...
- **#226 is decided everywhere and gated**, 2026-08-11. Every place a string a person typed reaches a screen is either isolated or carries `// bidi-ok:` and a reason on the line, in four kinds: a caller isolates one level up, the words come from the catalog rather than the family, the app formats it itself, or it is one of the three that must never be isolated at all, a field being edited, a draft on its way to the database, and anything building a filename. **`report_bidi_isolation.py` is now `check_bidi_isolation.py` and fails the build**, so what it catches from here is the next screen somebody writes. **A green check means every place has been decided, not that every decision is right**: only Arabic on the device shows that, and the capture form is the one walked so far.
...
**Never route around a check to make progress, and never delete or weaken a test to make a build pass.**
    ...
    cd android && ./gradlew :app:connectedDebugAndroidTest   # 598 tests, about eleven minutes
