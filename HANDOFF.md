...
- **18 repository checks pass**, `python3 tools/checks/run_all.py`. The eighteenth is `check_readable_labels.py`, which holds every table and column the archive renders to a word in all four catalogs, **and since 2026-08-10 also holds a `link` column's declared catalog to `templates/data`**, because a catalog lookup that guesses answers wrongly rather than failing. #329.
...
- **533 instrumented tests pass**, last full run 2026-08-10 on the unlocked phone, after #212's eleven landed.
