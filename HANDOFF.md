...
- **601 instrumented tests pass**, full run 2026-08-11 at 04:20 on the unlocked phone, with 218 unit tests and lint, through `tools/verify.sh --device`. **`RootStatesTest` is new**: the three states above the notebook, which nothing could compose until `RootState` stopped being private. **Six classes are new or much bigger**: `RemovalIsVisibleTest` seventeen, `FirstDaysTest` seven, `DocumentDateTest` four, `IncidentScreenTest` two, `OwnMeasureTest` three, and **`ScreenReaderTest` from 61 cases to 92**, covering every screen.
