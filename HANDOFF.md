...
**Last rewritten:** 2026-08-11 at dawn, after a long night: #218, #135, #339, #340, #341, #342 and #226 closed, #343 filed, and the audits on #204, #205 and #46.
...
- **603 instrumented tests pass**, full run 2026-08-11 at 04:52 on the unlocked phone, with 218 unit tests and lint, through `tools/verify.sh --device`. **`RootStatesTest` is new**: the three states above the notebook, which nothing could compose until `RootState` stopped being private. **Six classes are new or much bigger**: `RemovalIsVisibleTest` seventeen, `FirstDaysTest` seven, `DocumentDateTest` four, `IncidentScreenTest` two, `OwnMeasureTest` three, and **`ScreenReaderTest` from 61 cases to 95**, covering every screen. **The three newest are the states its own fixtures hid**: an export that failed, a person with nothing but a name, and a medication with nothing but a name. Every fixture in that file is a full record, deliberately, and the partially filled state fell through the middle of that decision.
...
- **The phone was returned to its starting values on 2026-08-11 and can be unplugged**, each setting read back rather than assumed: font scale 1.0, animator null, touch exploration 0, no per-app locale so the app runs in English, and the accessibility services string the KDE Connect one alone. **The font scale was changed to 2.0 twice during the night and restored both times**, which is rule 19's exception and nothing wider. **It holds the current build and the month six fixture notebook** with the disclaimer accepted, and none of it is real. All four attachment files present.
- **The owner is testing the build by hand, and a lot landed on 2026-08-11 that he has not seen.** In the order he is likeliest to meet them: **searching for a project by name now opens the project** rather than a screen saying it was not built; **long subtitles wrap instead of stopping mid-word**, which is most visible on the care team at a large font; **removal is now an outlined "Remove this" on each thing's own screen** rather than a long press; **a question that has not been asked opens** onto its own face; **a document is asked when it is from** and no longer stamped with today; **a new notebook gets a first days list** on Today from its setting's checklist; **Progress can track something he names himself**; and the foot of most screens now has its actions sized to their labels with only the way back full width. Before that, from 2026-08-10: the date picker's month and year views, the trail's kind filter, cards having a shadow in light theme, and the document folder field.
...
- **Every `DESIGN.md` section number in `CLAUDE.md` points at the numbering that existed before the v4 adoption**, checked one by one on 2026-08-11 rather than sampled. Rule 22 sends a session to section 11 for the component library and section 11 is Voice; the inventory is 7. **Nothing has been edited, because that file is the owner's**, and #344 carries the whole table plus the two pointers that have no target at all. **Follow the content, not the number**, until it is settled.
...
- **#226 is closed**, 2026-08-11. Every place a string a person typed reaches a screen is isolated or carries `// bidi-ok:` and a reason, and `check_bidi_isolation.py` fails the build on a new one. **A green check means every place has been decided, not that every decision is right**: only Arabic on the device shows that, the capture form is the one screen walked that way, and **the rest of it moved to #205**.
