...
**What is next on this track:** the importer's remaining rules in 8.3 (**#211**), which three other things are now waiting on: the fuller regeneration test, and the Numbers and Absence failure modes, both of which can only be caught on the import side. **#209, #213, #214 and #215 are done. #210 is not**, and an earlier line in this file said it was: the readable copy renders 61 pages with no base64 and no network, and **it never links an attachment**. `contract/DATA-CONTRACT.md` 8.2 requires every attachment referenced by relative path into `../attachments/`, and a real export was checked for `../attachments/` and has none. That is one of four things #210 still lists.
