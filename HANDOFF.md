...
- **The suite is owed a full run again**: **17 tests written since the 611 run have only had their own classes run**, all green. `CreateThreadTest` 4, `MedicationsScreenTest` 4, `MedicationQuestionCountsTest` 1, `CareTeamScreenTest` 4, `ChaptersScreenTest` 4, `AppointmentsMonthTest` 3, `IncidentCorrectionTest` 3, plus one case each added to `ScreenReaderTest` twice. **Expect 631 on the next `tools/verify.sh --device`**, and check rather than trust the arithmetic. **Three of those have never run at all**, `IncidentCorrectionTest`, because they were written after the phone was unplugged.
