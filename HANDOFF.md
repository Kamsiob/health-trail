...
- **The suite is owed a full run again**: **17 tests written since the 611 run have only had their own classes run**, all green. `CreateThreadTest` 4, `MedicationsScreenTest` 4, `MedicationQuestionCountsTest` 1, `CareTeamScreenTest` 4, `ChaptersScreenTest` 4, `AppointmentsMonthTest` 3, plus one case each added to `ScreenReaderTest` twice. **Expect 628 on the next `tools/verify.sh --device`**, and check rather than trust the arithmetic.
- **611 instrumented tests passed on the last full device run**, 2026-08-12 at 00:31, with 218 unit tests and lint, through `tools/verify.sh --device`, and 611 was exactly what #321 said to expect. **Nothing had landed unchecked.** The count is read from `android/app/build/outputs/androidTest-results/connected/debug/TEST-*.xml`, whose root element carries it: `verify.sh`'s own summary table lists the unit classes only. **Everything written after it is listed in the line above.**
...
- **The fidelity pass, #345, has now compared fourteen screens**, 2026-08-12 at 03:00, and **seven drifts are built and closed**: #347, #348, #351 the care team's lead group, #352 the medication row, Money's "Needs attention" eyebrow, #354 the emergency card as rows, #355 a way to write a question down, #356 the current chapter's counted line, and #357 the appointments month view. **#353 is filed and blocked on data.** Progress, Documents, the trail, capture, Today and the notebook all hold.
