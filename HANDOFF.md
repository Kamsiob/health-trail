...
**The Arabic one was attempted at the end of this run and abandoned**, which is worth knowing so nobody assumes it half exists. The app was switched to Arabic and an export started, and the system file picker's Save button is mirrored in a right to left layout, so the tap landed on nothing and no file was written. Nothing was left in a bad state; the locale was restored. **Doing it needs the picker's Save located by its own bounds rather than by a remembered position**, which is what `tools/walk.sh` does for the app's own screens and does not do for system dialogs.
