...
- **606 instrumented tests passed on the last full device run**, 2026-08-11, with 218 unit tests and lint, through `tools/verify.sh --device`. **Five tests written after that run have only had their own classes run**, green: `ThreadPositionTest` two and `UnsettledTotalTest` three. **A full run is owed the moment the phone is back**, and the expected total is 611. **`NotebookScreenTest` was rewritten** for the per-section counts and passed 14 of 14 on its own.
